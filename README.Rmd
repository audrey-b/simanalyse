...
Simulate 50 datasets using the sims package
```{r}
set.seed(10L)
params <- nlist(mu=0)
dat <- sims::sims_simulate("a ~ dnorm(mu,1)", parameters = params, nsims=50)
```

Analyse all 50 datasets (here we use only a few iterations for demonstration purposes)
```{r}
...
Summarize the results over the 50 datasets
```{r}
