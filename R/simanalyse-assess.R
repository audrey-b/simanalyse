...
sma_assess <- function(object,
  ...
  # discrepancy
  if (statistic == "FT") {
    fun.stat <- function(data, expectation) (sqrt(data) - sqrt(expectation))^2
  } else if (statistic == "LR") {
    fun.stat <- function(data, expectation) 2 * data * log(data / expectation)
  } else if (statistic == "chi2") {
    fun.stat <- function(data, expectation) (data - expectation)^2 / expectation # general case: denom is variance
  }
